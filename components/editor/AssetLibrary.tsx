            ...
            <div className="flex border-b border-white/10">
                {(['media', 'effects', 'text', 'audio'] as const).map((tab) => (
                    <button
                        ...
                        className={`flex-1 py-3 text-sm font-medium capitalize transition-all relative ${activeTab === tab
                                ? 'text-emerald-400'
            ...
            <div className="p-3 border-b border-white/10">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2 bg-black/30 border border-white/10 rounded-lg text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-emerald-500/50 transition-all"
                    />
                </div>
            </div>
                                ...
                                videos.map((video) => (
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            className="absolute top-1 right-1 p-1 bg-red-500/80 hover:bg-red-500 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10"
                                        >
                        ...
                        >
                            {effects.map((effect, i) => (
                                <motion.button
                                    key={i}
                                    whileHover={{ x: 4 }}
                                    className="w-full p-3 bg-black/30 border border-white/10 rounded-lg hover:border-emerald-500/50 transition-all flex items-center gap-3 group"
                                >
                                    <div className="p-2 bg-emerald-500/10 rounded-lg group-hover:bg-emerald-500/20 transition-colors">
                                        <effect.icon className="w-5 h-5 text-emerald-400" />
                                    </div>
                                    <span className="text-sm text-zinc-300 group-hover:text-white transition-colors">{effect.name}</span>
                        ...
                        >
                            {['Title', 'Subtitle', 'Lower Third', 'Credits'].map((preset, i) => (
                                <motion.button
                                    key={i}
                                    whileHover={{ x: 4 }}
                                    className="w-full p-4 bg-black/30 border border-white/10 rounded-lg hover:border-emerald-500/50 transition-all"
                                >
                        ...
                        >
                            {['Whoosh', 'Impact', 'Transition', 'Ambient'].map((sound, i) => (
                                <motion.button
                                    key={i}
                                    whileHover={{ x: 4 }}
                                    className="w-full p-3 bg-black/30 border border-white/10 rounded-lg hover:border-emerald-500/50 transition-all flex items-center gap-3"
                                >
